# Corner Portfolio — Ryan Jun / stray (Design)

**Date:** 2026-05-14
**Deadline:** May 23, 2026 networking event (9 days)
**Status:** Direction locked after 7+ prior iterations. Honest critique applied — terminal costume layer dropped in favor of unadorned monospace notes feed.
...
The branding is two-part: **Ryan Jun** as the principal name, **stray** as the studio identity (the multidisciplinary, raw-simplicity, quietly-connected sensibility). Both names visible, principal name leading.
...
None of them perform a "terminal" costume. All of them have **monospace or near-monospace typography, a vertical column, and writing**.
...
1. **Masthead** — `RYAN JUN — stray` (one line, mono, small caps lockup using existing `t-monument` or `t-statement` role).
...
4. **Notes feed** — reverse-chronological vertical list. Each note row is a single line:
   `§042  2026-05-14  process  on staying in one place ↗`
   Numbered, dated, category tag, title, arrow that indicates clickable detail.
5. **Footer / colophon** — quiet. `stray, an independent practice. ryan jun, principal. © 2026.` Live time. Theme toggle. Build hash micro-flourish.
...
`§NNN  YYYY-MM-DD  category  title  ↗`
- §NNN — monotonic note number (e.g. §001, §002, …)
...
- **`Masthead`** — renders `RYAN JUN — stray` lockup. Hosts the `rj-mark` view-transition-name for cross-page continuity. Pure presentation.
