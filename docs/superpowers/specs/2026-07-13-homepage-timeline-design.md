...
- **Native horizontal scroll container** (`overflow-x: auto`) holds the media row. Trackpad horizontal swipe and touch swipe work natively — no JS needed for those two input paths. The native scrollbar is hidden, matching the existing convention in `src/components/corner/ProjectSlider.tsx` (`.corner-slider__track`) — the tick-mark axis's progress indicator is the visible scroll indicator, so a native scrollbar underneath it would be redundant/conflicting.
...
- **Accessibility:**
  - `prefers-reduced-motion` disables the inertia *interpolation* only (direct snap to `target`, no eased approach) — it does not disable active-stop detection, the title update, or the `aria-live` announcement, which all keep running per the point above. Unlike `MotionReveal.tsx` (which gets reduced-motion for free from the global CSS rule, since it only ever sets a CSS `transition`), this component's easing is a hand-rolled JS `requestAnimationFrame` loop that the CSS rule cannot reach — it needs its own explicit `window.matchMedia('(prefers-reduced-motion: reduce)')` check to skip the interpolation.
  - The `aria-live="polite"` title announcement is throttled independently of the visual crossfade timing, so rapid inertia-driven scrolling through several stops doesn't queue up more announcements than a screen reader can usefully consume.
